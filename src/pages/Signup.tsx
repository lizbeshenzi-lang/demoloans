...
import { Mail, Lock, User, Loader2, Phone, CreditCard } from "lucide-react";
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: { full_name: fullName, national_id: nationalId },
      },
    });
    ...
    if (error) {
      ...
      toast({ title: "Check your email", description: "We sent a verification link to " + email + ". Please verify your email before signing in." });
    }
      ...
      <div className="w-full max-w-md">
        <div className="bg-background rounded-2xl shadow-elevated p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground font-body mt-1">Join Kechita Capital today</p>
          </div>

          {/* Google Signup */}
          <Button type="button" variant="outline" className="w-full mb-6" onClick={handleGoogleSignup} disabled={googleLoading}>
            {googleLoading ? (
          ...
          {authMethod === "email" ? (
            <form onSubmit={handleEmailSignup} className="space-y-4">
              <div>
                ...
                <div className="relative mt-1.5">
                  ...
                  <Input id="fullName" type="text" placeholder="John Kamau" value={fullName} onChange={(e) => setFullName(e.target.value)} className="pl-10" required />
                </div>
              </div>
              <div>
                <Label htmlFor="nationalId" className="font-body">National ID Number *</Label>
                <div className="relative mt-1.5">
                  <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <Input id="nationalId" type="text" placeholder="e.g. 12345678" value={nationalId} onChange={(e) => setNationalId(e.target.value)} className="pl-10" required maxLength={20} />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1.5 font-body">Must be at least 6 characters</p>
              </div>
            ...
            <form onSubmit={handlePhoneSignup} className="space-y-4">
              <div>
                <Label htmlFor="fullNamePhone" className="font-body">Full Name</Label>
                <div className="relative mt-1.5">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <Input id="fullNamePhone" type="text" placeholder="John Kamau" value={fullName} onChange={(e) => setFullName(e.target.value)} className="pl-10" required />
                </div>
              </div>
              <div>
                <Label htmlFor="nationalIdPhone" className="font-body">National ID Number *</Label>
                <div className="relative mt-1.5">
                  <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <Input id="nationalIdPhone" type="text" placeholder="e.g. 12345678" value={nationalId} onChange={(e) => setNationalId(e.target.value)} className="pl-10" required maxLength={20} />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative mt-1.5">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <Input id="phonePassword" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10" minLength={6} required />
                </div>
                <p className="text-xs text-muted-foreground mt-1.5 font-body">Must be at least 6 characters</p>
              </div>
              <Button type="submit" className="w-full bg-gradient-primary" disabled={loading}>
